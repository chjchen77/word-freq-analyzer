...
科研级中文文本词频统计工具。默认逐条保留原始记录，识别日期中的年份和月份，不按年份或月份合并；也可在界面中切换回「公司×年份」汇总模式。
...
1. **数据选择** — 添加文件夹，扫描文件，配置公司代码列、日期/年份列、文本列；支持点击文件预览前 100 行
2. **词典管理** — 导入或手动创建分类词典；支持多选批量删除、实时搜索、分类重命名、全选/清空本类
3. **分析设置** — 逐条保留或公司×年份汇总、匹配模式、输出选项、停用词、并发线程数、LLM 句子分析、输出路径
...
| v4.1 | 默认逐条保留原始记录，支持从日期提取月份；保留公司×年份汇总兼容模式 |
